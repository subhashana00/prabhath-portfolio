    ...
    <div className="min-h-screen bg-white">
      ...
      <section ref={designSystemRef} className="bg-[#FCF9F8] py-20 sm:py-28 border-t-4 border-black relative">
        <div className="container max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-12">
          ...
          {/* Color Palette */}
          <div className="bg-white border-4 border-black p-8 sm:p-10 shadow-[8px_8px_0_0_#000] mb-8">
            <h3 className="text-xl font-black uppercase mb-6 flex items-center gap-2">
              <div className="w-2 h-2 bg-[#007BFF] rounded-full"></div> Color Palette
            </h3>
            <p className="text-base font-medium text-gray-600 mb-6 leading-relaxed">
              Selected a professional color scheme anchored by deep blues and neutral grays to convey trust and data clarity. Accent colors (amber, green, red) serve as semantic cues for status indicators — keeping the interface informative without visual overload.
            </p>
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
              {[
                { color: '#1A1D2E', name: 'Dark Navy', usage: 'Sidebar / Headers' },
                { color: '#007BFF', name: 'Primary Blue', usage: 'CTAs / Links' },
                { color: '#00C9A7', name: 'Success Green', usage: 'Positive Status' },
                { color: '#FFB84D', name: 'Warning Amber', usage: 'Alerts / Pending' },
                { color: '#FF6B6B', name: 'Error Red', usage: 'Critical / Cancel' },
                { color: '#F5F6FA', name: 'Background', usage: 'Page / Card BG' },
              ].map((c, i) => (
                <div key={i} className="group">
                  <div className="aspect-square border-3 border-black shadow-[3px_3px_0_0_#000] group-hover:shadow-[5px_5px_0_0_#000] group-hover:-translate-y-1 transition-all duration-200" style={{ backgroundColor: c.color }}></div>
                  <div className="mt-2">
                    <div className="text-xs font-black text-black">{c.name}</div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase">{c.usage}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0_0_#000]">
              ...
              <div className="space-y-4">
                <div className="border-b-2 border-dashed border-gray-200 pb-4">
                  <div className="text-3xl font-black text-black">Inter / Bold</div>
                  <div className="text-xs font-bold text-gray-400 uppercase mt-1">Primary Headings</div>
                </div>
                <div className="border-b-2 border-dashed border-gray-200 pb-4">
                  <div className="text-xl font-semibold text-gray-700">Inter / Semi-Bold</div>
                  <div className="text-xs font-bold text-gray-400 uppercase mt-1">Subheadings & Labels</div>
                </div>
                <div>
                  <div className="text-base font-medium text-gray-500">Inter / Regular</div>
                  <div className="text-xs font-bold text-gray-400 uppercase mt-1">Body & Descriptions</div>
                </div>
              </div>
              <p className="text-sm font-medium text-gray-500 mt-6 leading-relaxed">
                Inter was chosen for its excellent legibility at small sizes — critical for data-dense dashboard interfaces where every pixel of clarity matters.
              </p>
            </div>

            <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0_0_#000]">
              <h3 className="text-xl font-black uppercase mb-6 flex items-center gap-2">
                <div className="w-2 h-2 bg-[#A0E7E5] rounded-full"></div> Key Components
              </h3>
              <div className="space-y-3">
                {[
                  { name: "KPI Metric Cards", desc: "Large numbers with trend indicators and sparklines" },
                  { name: "Data Tables", desc: "Sortable, filterable with inline actions" },
                  { name: "Chart Modules", desc: "Bar, line, donut charts with hover tooltips" },
                  { name: "Status Badges", desc: "Semantic color-coded pills for event states" },
                  { name: "Filter Controls", desc: "Dropdowns, date pickers, search inputs" },
                  { name: "Navigation Sidebar", desc: "Collapsible with icon + label states" },
                ].map((comp, i) => (
                  <div key={i} className="flex items-center gap-3 bg-[#FCF9F8] border-2 border-black p-3">
                    <div className="w-1.5 h-1.5 bg-black rounded-full shrink-0"></div>
                    <div>
                      <span className="text-sm font-black text-black">{comp.name}</span>
